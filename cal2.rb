...
table = Terminal::Table.new :title => "Convert", :headings => ['DAY', 'CELSIUS', 'FAHRENHEIT'] do |t|
  Day.all.each do |day|
     day.temperature > 29 ? color = :red : color = :blue
    t << [day.name, Paint[day.temperature, color], Paint[day.temperature("fahrenheit"), color]]
  end
end
